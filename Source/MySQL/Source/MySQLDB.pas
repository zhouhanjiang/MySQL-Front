...
type
  ...
  type
    ...
    private
      ...
      FConnection: TMySQLConnection;
      {$IFDEF Log}
      Monitor: TMySQLMonitor;
      MonitorCS: TCriticalSection;
      {$ENDIF}
      RunExecute: TEvent;
...
{$IFDEF Log}
procedure TMySQLConnection.TSyncThread.AppendLog(const Text: string);
begin
  MonitorCS.Enter();
  Monitor.Append(Text + ', State: ' + IntToStr(Ord(State)) + ', ID: ' + IntToStr(GetCurrentThreadId()), ttDebug);
  MonitorCS.Leave();
end;
{$ENDIF}
...
begin
  ...
  {$IFDEF Log}
  Monitor := TMySQLMonitor.Create(nil);
  MonitorCS := TCriticalSection.Create();
  Monitor.TraceTypes := [ttDebug];
  Monitor.CacheSize := 2000;
  AppendLog('Create()');
  {$ENDIF}
...
{$IFDEF Log}
function TMySQLConnection.TSyncThread.GetLog(): string;
begin
  MonitorCS.Enter();
  Result := Monitor.CacheText;
  MonitorCS.Leave();
end;
{$ENDIF}
    ...
    case (State) of
      ...
      ssExecutingNext:
        begin
          ...
          if ((Connection.KillThreadId > 0) and (Mode = smResultHandle)) then
          begin
...
begin
  ...
  begin
    ResultHandle.Done := TEvent.Create(nil, False, False, '');
...
begin
  // Debug 2017-01-02
  if (Assigned(ResultHandle.SyncThread) and (ResultHandle.SyncThread.State = ssResult)) then
    raise ERangeError.Create('SyncThread: ' + IntToStr(ResultHandle.SyncThread.ThreadID));
  ...
  begin
    InternExecuteSQL(smResultHandle, RightStr(ResultHandle.SQL, Length(ResultHandle.SQL) - (ResultHandle.SQLIndex - 1)), TResultEvent(nil), ResultHandle.Done);
    ResultHandle.Done.WaitFor(INFINITE);
...
begin
  ...
  {$IFDEF Log}
  SyncThread.AppendLog('SyncAfterExecuteSQL - start');
  {$ENDIF}
  ...
  {$IFDEF Log}
  SyncThread.AppendLog('SyncAfterExecuteSQL - end');
  {$ENDIF}
...
begin
  {$IFDEF Log}
  SyncThread.AppendLog('SyncBeforeExecuteSQL - start');
  {$ENDIF}

  DoBeforeExecuteSQL();

  {$IFDEF Log}
  SyncThread.AppendLog('SyncBeforeExecuteSQL - end');
  {$ENDIF}
...
begin
  ...
  {$IFDEF Log}
  SyncThread.AppendLog('SyncBindDataSet - start');
  {$ENDIF}
  ...
  {$IFDEF Log}
  SyncThread.AppendLog('SyncBindDataSet - end');
  {$ENDIF}
...
begin
  {$IFDEF Log}
  SyncThread.AppendLog('SyncConnecting - start');
  {$ENDIF}
  ...
  {$IFDEF Log}
  SyncThread.AppendLog('SyncConnecting - end');
  {$ENDIF}
end;
...
begin
  {$IFDEF Log}
  SyncThread.AppendLog('SyncConnected - start');
  {$ENDIF}
  ...
  {$IFDEF Log}
  SyncThread.AppendLog('SyncConnected - end');
  {$ENDIF}
end;
...
begin
  ...
  {$IFDEF Log}
  SyncThread.AppendLog('SyncDisconnecting - start');
  {$ENDIF}
  ...
  {$IFDEF Log}
  SyncThread.AppendLog('SyncDisconnecting - end');
  {$ENDIF}
end;
...
begin
  ...
  {$IFDEF Log}
  SyncThread.AppendLog('SyncExecute - start');
  {$ENDIF}
  ...
  {$IFDEF Log}
  SyncThread.AppendLog('SyncExecute - end');
  {$ENDIF}
...
begin
  ...
  {$IFDEF Log}
  SyncThread.AppendLog('SyncExecuted - start');
  {$ENDIF}
  ...
  {$IFDEF Log}
  SyncThread.AppendLog('SyncExecuted - end');
  {$ENDIF}
...
begin
  ...
  {$IFDEF Log}
  SyncThread.AppendLog('SyncExecutingNext - start');
  {$ENDIF}
...
begin
  ...
  {$IFDEF Log}
  SyncThread.AppendLog('SyncHandledResult - end');
  {$ENDIF}
end;
...
begin
  ...
  {$IFDEF Log}
  SyncThread.AppendLog('SyncReceivingResult - start');
  {$ENDIF}
  ...
  {$IFDEF Log}
  SyncThread.AppendLog('SyncReceivingResult - end');
  {$ENDIF}
end;
...
begin
  if (not Active) then
    ...
    raise ERangeError.Create('Index: ' + IntToStr(InternRecordBuffers.IndexOf(ActiveBuffer())) + #13#10
      + 'Index: ' + IntToStr(InternRecordBuffers.IndexOf(PExternRecordBuffer(ActiveBuffer())^.InternRecordBuffer)) + #13#10
      ...
      + 'IdentifierABCDEF: ' + IntToStr(PExternRecordBuffer(ActiveBuffer())^.InternRecordBuffer^.IdentifierABCDEF))
  else if (not Assigned(PExternRecordBuffer(ActiveBuffer())^.InternRecordBuffer^.NewData)) then
  else if (PExternRecordBuffer(ActiveBuffer())^.InternRecordBuffer^.NewData^.Identifier123456 <> 123456) then
...
begin
  if (PExternRecordBuffer(ActiveBuffer())^.BookmarkFlag <> bfCurrent) then
    ...
    Result := InternRecordBuffers.IndexOf(ActiveBuffer());
end;
...
begin
  if (PExternRecordBuffer(ActiveBuffer())^.BookmarkFlag <> bfCurrent) then
  begin
    Index := InternRecordBuffers.IndexOf(ActiveBuffer());
...
begin
  ...
  begin
    ...
    if (PExternRecordBuffer(ActiveBuffer())^.InternRecordBuffer^.IdentifierABCDEF <> $ABCDEF) then
      raise ERangeError.Create('BookmarkFlag: ' + IntToStr(Ord(PExternRecordBuffer(ActiveBuffer())^.BookmarkFlag)));
      // Occurred 2017-01-05: bfCurrent
    ...
    begin
      ...
      try
        if (not Connection.ExecuteSQL(SQL, InternalPostResult)) then
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          else if (Index > InternRecordBuffers.IndexOf(ActiveBuffer())) then
            Dec(Index);
        end
        else
          Index := InternRecordBuffers.Count - 1;

        if (Index <> InternRecordBuffers.IndexOf(ActiveBuffer())) then
        begin
          InternRecordBuffers.Move(InternRecordBuffers.IndexOf(ActiveBuffer()), Index);
...
var
  Index: Integer;
begin
  Index := InternRecordBuffers.IndexOf(Buffer);

  if (Index < 0) then
  begin
    Index := InternRecordBuffers.IndexOf(PExternRecordBuffer(Buffer)^.InternRecordBuffer);

    if (Index < 0) then
      raise ERangeError.Create('BookmarkFlag: ' + IntToStr(Ord(PExternRecordBuffer(Buffer)^.BookmarkFlag)))
    else
      SendToDeveloper('InternRecordBuffers.Count: ' + IntToStr(InternRecordBuffers.Count) + #13#10
        + 'InternRecordBuffers.Index: ' + IntToStr(InternRecordBuffers.Index) + #13#10
        + 'Index: ' + IntToStr(Index) + #13#10
        + 'BufferCount: ' + IntToStr(BufferCount) + #13#10
        + 'ActiveRecord: ' + IntToStr(ActiveRecord) + #13#10
        + 'BookmarkFlag: ' + IntToStr(Ord(PExternRecordBuffer(Buffer)^.BookmarkFlag)), 2);
  end;
...
begin
  // Why is this needed in Delphi XE4? Without this, Buffers are not reinitialized well.
  if ((InternRecordBuffers.Index >= 0) and (InternRecordBuffers.IndexOf(ActiveBuffer()) >= 0)
    and Assigned(PExternRecordBuffer(ActiveBuffer())^.InternRecordBuffer)
    and (Mode = [])) then
      InternRecordBuffers.Index := InternRecordBuffers.IndexOf(ActiveBuffer());
