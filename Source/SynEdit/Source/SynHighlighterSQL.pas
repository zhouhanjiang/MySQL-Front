...
//---MySQL----------------------------------------------------------------------
  ...
  MySqlKW: UnicodeString =
    'ACCOUNT,ACTION,AFTER,AGAINST,AGGREGATE,ALGORITHM,ALL,ALTER,ALWAYS,' +
    'ANALYZE,AND,ANY,AS,ASC,AT,AUTHORS,AUTO_INCREMENT,AUTOEXTEND_SIZE,' +
    'AVG_ROW_LENGTH,BACKUP,BEFORE,BEGIN,BENCHMARK,BETWEEN,BINLOG,BIT,BLOCK,' +
    'BOTH,BY,CACHE,CALL,CASCADE,CASCADED,CHAIN,CHANGE,CHANGED,CHARACTER,' +
    'CHARSET,CHECK,CHECKSUM,CLIENT,CODE,COLLATE,COLLATION,COLUMN,' +
    'COLUMN_FORMAT,COLUMNS,COMMENT,COMMIT,COMMITTED,COMPLETION,CONCURRENT,' +
    'CONNECTION,CONSISTENT,CONSTRAINT,CONTAINS,CONTENTS,CONTEXT,CONTRIBUTORS,' +
    'CONVERT,COPY,CPU,CREATE,CROSS,CURRENT,DATA,DATABASE,DATABASES,DATAFILE,' +
    'DEALLOCATE,DEC,DEFAULT,DEFINER,DELAY_KEY_WRITE,DELAYED,DELETE,DESC,' +
    'DESCRIBE,DETERMINISTIC,DIAGNOSTICS,DIRECTORY,DISABLE,DISCARD,DISTINCT,' +
    'DISTINCTROW,DIV,DROP,DUAL,DUMPFILE,DUPLICATE,EACH,ELSE,ELSEIF,ENABLE,' +
    'ENCLOSED,END,ENDIF,ENDS,ENGINE,ENGINES,ERRORS,ESCAPE,ESCAPED,EVENT,' +
    'EVENTS,EVERY,EXCHANGE,EXCLUSIVE,EXECUTE,EXISTS,EXPANSION,EXPIRE,EXPLAIN,' +
    'EXTENDED,EXTENT_SIZE,FALSE,FAST,FAULTS,FIELDS,FILE,FIRST,FLUSH,FOR,' +
    'FORCE,FOREIGN,FROM,FULL,FULLTEXT,FUNCTION,FUNCTIONS,GENERATED,' +
    'GET,GLOBAL,GRANT,GRANTS,GROUP,HAVING,HELP,HIGH_PRIORITY,HOST,HOSTS,' +
    'IDENTIFIED,IGNORE,IGNORE_SERVER_IDS,IMPORT,IN,INDEX,INDEXES,INFILE,' +
    'INITIAL_SIZE,INNER,INOUT,INPLACE,INSERT,INSERT_METHOD,INSTALL,INT1,INT2,' +
    'INT3,INT4,INT8,INTO,INVOKER,IO,IO_THREAD,IPC,IS,ISOLATION,ISSUER,JOIN,' +
    'KEY,KEY_BLOCK_SIZE,KEYS,KILL,LAST,LEADING,LEAVES,LESS,LEVEL,LIKE,LIMIT,' +
    ...
    'MIGRATE,MIN_ROWS,MOD,MODE,MODIFIES,MODIFY,MUTEX,NAME,NAMES,NATURAL,' +
    'NEVER,NEW,NEXT,NO,NO_WRITE_TO_BINLOG,NODEGROUP,NONE,NOT,NULL,NUMBER,' +
    'OFFLINE,OFFSET,OJ,OLD,ON,ONE,ONLINE,ONLY,OPEN,OPTIMIZE,OPTION,OPTIONAL,' +
    'OPTIONALLY,OPTIONS,OR,ORDER,OUT,OUTER,OUTFILE,OWNER,PACK_KEYS,PAGE,' +
    'PAGE_CHECKSUM,PARSER,PARTIAL,PARTITION,PARTITIONING,PARTITIONS,PHASE,' +
    'PLUGIN,PLUGINS,PORT,PREPARE,PRESERVE,PREV,PRIMARY,PRIVILEGES,PROCEDURE,' +
    'PROCESS,PROCESSLIST,PROFILE,PROFILES,PROXY,PURGE,QUERY,QUICK,' +
    'RAID_CHUNKS,RAID_CHUNKSIZE,RAID_TYPE,RANGE,READ,READS,REBUILD,RECOVER,' +
    'REDO_BUFFER_SIZE,REFERENCES,REGEXP,RELAY_LOG_FILE,RELAY_LOG_POS,' +
    'RELAYLOG,RELEASE,RELOAD,REMOVE,RENAME,REORGANIZE,REPAIR,REPEATABLE,' +
    'REPLACE,REPLICATION,REQUIRE,RESET,RESIGNAL,RESTORE,RESTRICT,RESUME,' +
    'RETURN,RETURNED_SQLSTATE,RETURNS,REVOKE,RLIKE,ROLLBACK,ROLLUP,ROUTINE,' +
    'ROW,ROW_FORMAT,ROWS,SAVEPOINT,SCHEDULE,SCHEMA,SCHEMAS,SECURITY,SELECT,' +
    'SEPARATOR,SERIALIZABLE,SERVER,SESSION,SET,SHARE,SHARED,SHOW,SHUTDOWN,' +
    'SIMPLE,SLAVE,SNAPSHOT,SOCKET,SONAME,SOUNDS,SOURCE,SQL,SQL_BIG_RESULT,' +
    'SQL_BUFFER_RESULT,SQL_CACHE,SQL_CALC_FOUND_ROWS,SQL_NO_CACHE,' +
    'SQL_SMALL_RESULT,SQL_THREAD,SQLWARNINGS,SSL,STACKED,START,STARTING,' +
    'STARTS,STATS_AUTO_CALC,STATS_AUTO_RECALC,STATS_PERSISTENT,STATUS,STOP,' +
    'STORAGE,STORED,STRAIGHT_JOIN,SUBJECT,SUBPARTITION,SUBPARTITIONS,SUPER,' +
    'SUSPEND,SWAPS,SWITCHES,TABLE,TABLES,TABLESPACE,TEMPORARY,TERMINATED,' +
    'THAN,THEN,TO,TRADITIONAL,TRAILING,TRANSACTION,TRIGGER,TRIGGERS,TRUE,' +
    'TYPE,UNCOMMITTED,UNDO,UNDO_BUFFER_SIZE,UNDOFILE,UNINSTALL,UNION,UNIQUE,' +
    'UNKNOWN,UNLOCK,UPDATE,UPGRADE,USAGE,USE,USE_FRM,USING,VALUE,VALUES,' +
    'VARIABLES,VARYING,VIEW,VIRTUAL,WAIT,WARNINGS,WHERE,WITH,WORK,WRAPPER,' +
    'WRITE,X509,XA,XID,XML,XOR';

  // PLSQL keywords
  MySQLPLSQLKW: UnicodeString =
    'CASE,CATALOG_NAME,CLASS_ORIGIN,CLOSE,COLUMN_NAME,CONDITION,' +
    ...
    'IF,LEAVE,LOOP,MESSAGE_TEXT,MYSQL_ERRNO,REPEAT,SCHEMA_NAME,SIGNAL,SQLEXCEPTION,' +
    'SQLSTATE,SQLWARNING,SUBCLASS_ORIGIN,TABLE_NAME,UNTIL,WHEN,WHILE';
  ...
  MySQLTypes: UnicodeString =
    ...
    'armscii8_bin,armscii8_general_ci,ascii_bin,ascii_general_ci,big5_bin,' +
		...
		'utf8_icelandic_ci,utf8_latvian_ci,utf8_lithuanian_ci,' +
		'utf8_persian_ci,utf8_polish_ci,utf8_romanian_ci,utf8_roman_ci,' +
