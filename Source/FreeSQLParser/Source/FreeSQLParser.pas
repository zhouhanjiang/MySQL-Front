...
begin
  ...
  Result := nil;
  while (Assigned(Node) and Parser.IsStmtNode(Node) and (PStmtNode(Node) <> PStmtNode(ParentNode))) do
  begin
    Result := Node;
      ...
      if (not Error) then
        ...
        begin
          ...
          if (CurrentToken = 0) then
            ...
            repeat
              PPLSQLCondPart(NodePtr(Part))^.AddStmt(ParseStmt());
...
begin
  ...
  else if (KeywordIndex = kiIF) then
    Result := ParseIfStmt()
