...
type
  ...
  public
    ...
    property SItems: TSItems read FSItems;
  end;
  ...
  private
    ...
    procedure SetDefaultCharset(const ADefaultCharset: TSCharset);
  protected
  ...
  public
    ...
    property DefaultCharset: TSCharset read FDefaultCharset write SetDefaultCharset;
    property DefaultCodePage: Cardinal read FDefaultCodePage;
...
begin
  Result := SItems.IndexOf(Self);
end;

function TSItem.GetSession(): TSSession;
begin
  Result := SItems.Session;
end;
...
begin
  if (AName <> FName) then
  begin
    if (SItems.InsertIndex(AName, NewIndex) and (Index >= 0)) then
    begin
      if (NewIndex > Index) then
        Dec(NewIndex);
      SItems.Move(Index, NewIndex);
    end;
    FName := AName;
  end;
end;
...
begin
  Assert(SItems is TSEntities);

  Result := TSEntities(SItems);
end;
...
constructor TSObject.Create(const ASItems: TSItems; const AName: string = '');
begin
  inherited;
...
begin
  Assert(SItems is TSObjects);

  Result := TSObjects(SItems);
end;
...
begin
  Assert(SItems is TSDBObjects);

  Result := TSDBObjects(SItems);
end;
...
begin
  ...
  begin
    if (SItemsEvents) then
      Session.ExecuteEvent(etItemsValid, Database, SItems);
    Session.ExecuteEvent(etItemValid, Database, SItems, Self);
  end;
end;
...
begin
  ...
  begin
    ...
    if (Self is TSTable) then
      FSItems := ADatabase.Tables
    else if (Self is TSRoutine) then
      FSItems := ADatabase.Routines
    else if (Self is TSRoutine) then
      FSItems := ADatabase.Routines
    else if (Self is TSTrigger) then
      FSItems := ADatabase.Triggers
    else
      raise ERangeError.Create(SRangeError);
...
begin
  Assert(SItems is TSKeys);

  Result := TSKeys(SItems);
end;
...
begin
  Assert(SItems is TSForeignKeys);

  Result := TSForeignKeys(SItems);
end;
...
begin
  ...
  begin
    if (Assigned(DataSet.FindField('Type'))) then // MySQL < 4.1.2 and 5.0.0???
      FEngine := Session.EngineByName(DataSet.FieldByName('Type').AsString)
    else
  ...
  begin
    ...
    if (not Assigned(Session.CharsetByCollation(FCollation))) then
      DefaultCharset := ''
    else
      DefaultCharset := Session.CharsetByCollation(FCollation).Name;
  end;
...
begin
  ...
  FDataSize := -1;
  if (Assigned(Database)) then
    FDefaultCharset := Database.DefaultCharset.Name
  else
    FDefaultCharset := '';
...
begin
  ...
  begin
    ...
    // Remove deleted (and renamed) fields to reduce ExecuteEvent for "moved"
    // fields
    ...
    for I := DeleteList.Count - 1 downto 0 do
    begin
      Session.ExecuteEvent(etItemDropped, Self, FFields, DeleteList[I]);
...
begin
  ...
  if (lstrcmpi(PChar(ADefaultCharset), 'binary') = 0) then
    FDefaultCodePage := 0
  else if (not Assigned(Database) or not Assigned(Session)) then
    FDefaultCodePage := CP_ACP
  else if (FDefaultCharset <> '') then
    FDefaultCodePage := Session.Connection.CharsetToCodePage(FDefaultCharset)
  else if (Database.DefaultCharset.Name <> '') then
    FDefaultCodePage := Database.DefaultCodePage
  else
    FDefaultCodePage := Session.Connection.CharsetToCodePage(Session.DefaultCharset)
end;
    ...
    begin
      for I := 0 to List.Count - 1 do
        ...
        begin
          ...
          Result := Result + SQLEscape(TSBaseTable(List[I]).Name);
        end;
      if (Result <> '') then
...
begin
  Assert(SItems is TSRoutines);

  Result := TSRoutines(SItems);
end;
...
begin
  Assert(SItems is TSTriggers);

  Result := TSTriggers(SItems);
end;
...
begin
  Assert(SItems is TSEvents);

  Result := TSEvents(SItems);
end;
...
begin
  Assert(SItems is TSDatabases);

  Result := TSDatabases(SItems);
end;
...
begin
  ...
  begin
    ...
    if (SQLParseKeyword(Parse, 'DEFAULT CHARACTER SET') or SQLParseKeyword(Parse, 'CHARACTER SET')) then
      DefaultCharset := Session.CharsetByName(SQLParseValue(Parse))
    else
      DefaultCharset := nil;

    if (SQLParseKeyword(Parse, 'DEFAULT COLLATE') or SQLParseKeyword(Parse, 'COLLATE')) then
      FCollation := LowerCase(SQLParseValue(Parse))
    else if (Assigned(DefaultCharset)) then
      FCollation := DefaultCharset.DefaultCollation.Caption
    else
      FCollation := '';
  end;
end;
...
procedure TSDatabase.SetDefaultCharset(const ADefaultCharset: TSCharset);
begin
  FDefaultCharset := ADefaultCharset;

  if (not Assigned(Session)) then
    FDefaultCodePage := CP_ACP
  else
    FDefaultCodePage := Session.Connection.CharsetToCodePage(DefaultCharset.Name);
end;
  ...
  begin
    ...
    repeat
      ...
      begin
        ...
        begin
          Database[Index].DefaultCharset := Session.CharsetByName(DataSet.FieldByName('DEFAULT_CHARACTER_SET_NAME').AsString);
...
begin
  Assert(SItems is TSVariables);

  Result := TSVariables(SItems);
end;
...
begin
  Assert(SItems is TSEngines);

  Result := TSEngines(SItems);
end;
...
begin
  Assert(SItems is TSPlugins);

  Result := TSPlugins(SItems);
end;
...
begin
  if (not (SItems is TSFieldTypes)) then
    Result := nil
  else
    Result := TSFieldTypes(SItems);
end;
...
begin
  Assert(SItems is TSCharsets);

  Result := TSCharsets(SItems);
end;
...
begin
  Assert(SItems is TSCollations);

  Result := TSCollations(SItems);
end;
...
begin
  ...
  if (not Assigned(FSItems)) then FSItems := Source.Users;
...
begin
  Assert(SItems is TSUsers);

  Result := TSUsers(SItems);
end;
...
begin
  ...
  Session.ExecuteEvent(etAfterExecuteSQL);
end;

procedure TSConnection.DoBeforeExecuteSQL();
begin
  Session.ExecuteEvent(etBeforeExecuteSQL);
...
procedure TSSession.ExecuteEvent(const EventType: TEvent.TEventType);
var
  Event: TEvent;
begin
...
procedure TSSession.ExecuteEvent(const EventType: TEvent.TEventType; const Sender: TObject; const SItems: TSItems = nil; const SItem: TSItem = nil);
var
  Event: TEvent;
begin
  Event := TEvent.Create(Self);
...
begin
  ...
  if (Connection.ServerVersion >= 40101) then
  begin
    if (Assigned(NewDatabase.DefaultCharset)) then
      SQL := SQL + ' DEFAULT CHARACTER SET ' + NewDatabase.DefaultCharset.Name;
